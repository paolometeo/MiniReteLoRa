...
*   TTGO LoRa32 SX1276          TTGO LoRa32-OLED        USB micro
 *               
 *  Setting for different boards are in Radio.h           
 *  
 *  Paolo Bonelli 18/04/2021
    Paolo Bonelli  28/01/2023   Reso compatibile con scheda ESP32
*/
...
void setup() {
  if (ECHO) {
 if(BOARD == 4)while (!Serial);
    Serial.begin(9600);
    delay(100);
  }
  //  Initializing the radio
  if (ECHO) Serial.println("inizializzo Radio");
  RadioInit();
  Serial.println("TX_Generic_esp32");
